...
public class AuthController {
	...
	public ResponseEntity<UserLoginPostRes> loginUser(
		...
		// 가입된 유저가 아닌 경우
		if (user == null)
			return ResponseEntity.status(404).body(UserLoginPostRes.of(404, "Unsubscribed User", null));
	...
	@ApiResponses({ @ApiResponse(code = 200, message = "성공", response = UserLoginPostRes.class),
			...
			@ApiResponse(code = 500, message = "서버 오류", response = BaseResponseBody.class) })
	public ResponseEntity<UserLoginPostRes> loginCounselor(
		...
		// 가입된 상담사가 아닌 경우
		if (counselor == null)
			return ResponseEntity.status(404).body(UserLoginPostRes.of(404, "Unsubscribed Counselor", null));
		...
		if (passwordEncoder.matches(password, counselor.getPassword())) {
			// 유효한 패스워드가 맞는 경우, 로그인 성공으로 응답.(액세스 토큰을 포함하여 응답값 전달)
			return ResponseEntity.ok(UserLoginPostRes.of(200, "Success", JwtTokenUtil.getToken(counselorId)));
		}
		// 유효하지 않는 패스워드인 경우, 로그인 실패로 응답.
		return ResponseEntity.status(401).body(UserLoginPostRes.of(401, "Invalid Password", null));
	}
